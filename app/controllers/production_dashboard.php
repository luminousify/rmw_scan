...
if (isset($_GET['action']) && $_GET['action'] === 'get_request_details') {
    ...
    try {
        ...
        // Production users can only view requests from their own division
        $query = "
            SELECT mr.*, u.full_name as production_user_name, u.department as production_department, u.division as production_division
        ...
        error_log("Production Dashboard AJAX Error: " . $e->getMessage() . " in " . $e->getFile() . ":" . $e->getLine());
        ...
        error_log("Production Dashboard System Error: " . $e->getMessage() . " in " . $e->getFile() . ":" . $e->getLine());
    ...
    try {
        ...
        if ($_POST['action'] === 'update_status') {
            ...
            $stmt = $pdo->prepare("
                ...
                SET status = ?, production_user_id = ?, processed_date = ?, completed_date = ?, processed_by = ?, updated_at = CURRENT_TIMESTAMP
                WHERE id = ?
            ");
            
            $processedDate = ($newStatus === 'diproses') ? date('Y-m-d H:i:s') : null;
            $completedDate = ($newStatus === 'completed') ? date('Y-m-d H:i:s') : null;
            $stmt->execute([$newStatus, $idlog, $processedDate, $completedDate, $processedBy, $requestId]);
...
include '../production_dashboard.php';
